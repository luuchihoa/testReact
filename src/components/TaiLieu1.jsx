...
import {
  GraduationCap, BookOpen, FileText, Play,
  Search, ChevronRight, Download, Clock,
  Sparkles, Flame, Heart, Star, Globe,
} from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

const fadeUp = {
  hidden:  { opacity: 0, y: 20 },
  visible: (d = 0) => ({ opacity: 1, y: 0, transition: { duration: 0.6, ease: [0.16, 1, 0.3, 1], delay: d } }),
};

/* ── Danh mục tài liệu theo khối ── */
const KHOI_LIST = [
  { id: "all",       label: "Tất cả" },
...
/* ── Quiz (ôn tập) — link đến TestQuiz route ── */
const QUIZZES = [
  {
...
/* ── Icon mapping theo khối ── */
const KHOI_ICON_MAP = {
  "chien-con":  { icon: Heart,      color: "text-rose-500",   bg: "bg-rose-50" },
  "ruoc-le":    { icon: Star,       color: "text-amber-500",  bg: "bg-amber-50" },
  "kinh-thanh": { icon: BookOpen,   color: "text-green-600",  bg: "bg-green-50" },
  "phung-vu":   { icon: Sparkles,   color: "text-violet-600", bg: "bg-violet-50" },
  "them-suc":   { icon: Flame,      color: "text-orange-600", bg: "bg-orange-50" },
  "vao-doi":    { icon: Globe,      color: "text-blue-600",   bg: "bg-blue-50" },
    ...
    <div className="min-h-screen bg-[#faf8f5] text-stone-900 antialiased overflow-x-hidden selection:bg-amber-200 selection:text-amber-900">
      ...
      <div className="max-w-5xl mx-auto px-5 sm:px-6 py-12 space-y-16">
        ...
        <section>
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-xl font-bold text-stone-900">Đề ôn luyện trực tuyến</h2>
            </div>
            ...
            {filteredQuizzes.length > 0 ? (
              <motion.div key={activeKhoi + search}
                ...
                {filteredQuizzes.map((quiz, i) => {
                  ...
                  return (
                    <motion.div key={quiz.path} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                      transition={{ duration: 0.4, delay: i * 0.06 }}>
                      <Link to={quiz.path} className="group flex flex-col h-full bg-white rounded-2xl border border-stone-100 p-5 shadow-sm hover:shadow-md hover:border-amber-300 transition-all">
        ...
        <section>
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-xl font-bold text-stone-900">Tài liệu tham khảo</h2>
            </div>
            ...
            {filteredDocs.length > 0 ? (
              <motion.div key={activeKhoi + search + "d"}
                ...
                {filteredDocs.map((doc, i) => {
                  ...
                  return (
                    <motion.div key={doc.title} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                      transition={{ duration: 0.4, delay: i * 0.06 }}>
                      <a href={doc.url}
                        className="group flex items-center gap-4 bg-white rounded-2xl border border-stone-100 px-5 py-4 shadow-sm hover:shadow-md hover:border-stone-300 transition-all">
                        ...
                        <div className="flex items-center gap-3 flex-shrink-0">
                          ...
                          <div className="w-8 h-8 rounded-full bg-stone-50 group-hover:bg-amber-50 border border-stone-200 group-hover:border-amber-200 flex items-center justify-center transition-colors">
                            <Download className="w-3.5 h-3.5 text-stone-400 group-hover:text-amber-600 transition-colors" />
                          </div>
                        </div>
                      </a>
                    </motion.div>
        ...
        <section>
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }} transition={{ duration: 0.6 }} className="mb-8">
            <h2 className="text-xl font-bold text-stone-900">Khám phá theo khối</h2>
            <p className="text-sm text-stone-500 mt-1">Tìm hiểu chi tiết chương trình của từng khối giáo lý.</p>
          </motion.div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
            {[
              { path: "/khối-chiên-con",   label: "Chiên Con",   icon: Heart,      color: "text-rose-500",   bg: "bg-rose-50",    border: "border-rose-100" },
              { path: "/khối-rước-lễ",     label: "Rước Lễ",     icon: Star,       color: "text-amber-500",  bg: "bg-amber-50",   border: "border-amber-100" },
              { path: "/khối-kinh-thánh",  label: "Kinh Thánh",  icon: BookOpen,   color: "text-green-600",  bg: "bg-green-50",   border: "border-green-100" },
              { path: "/khối-phụng-vụ",    label: "Phụng Vụ",    icon: Sparkles,   color: "text-violet-600", bg: "bg-violet-50",  border: "border-violet-100" },
              { path: "/khối-thêm-sức",    label: "Thêm Sức",    icon: Flame,      color: "text-orange-600", bg: "bg-orange-50",  border: "border-orange-100" },
              { path: "/khối-vào-đời",     label: "Vào Đời",     icon: Globe,      color: "text-blue-600",   bg: "bg-blue-50",    border: "border-blue-100" },
            ].map((k, i) => {
              const Icon = k.icon;
              return (
                <motion.div key={k.path} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.07 }}>
                  <Link to={k.path}
                    className={`group flex flex-col items-center gap-2 p-4 rounded-2xl border ${k.bg} ${k.border} hover:shadow-md transition-all text-center`}>
                    <div className={`w-10 h-10 rounded-xl bg-white flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform`}>
                      <Icon className={`w-5 h-5 ${k.color}`} />
                    </div>
                    <span className="text-xs font-bold text-stone-800">{k.label}</span>
                  </Link>
                </motion.div>
              );
            })}
