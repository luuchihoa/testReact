...
(async () => {
    ...
    for (let p of paragraphs) {
      ...
      if (isCapturingTitle) {
        if (lowerText.startsWith('bài đọc') || 
            ...
            lowerText.match(/^(✠\s*)?tin mừng chúa giê-su/i)) {
      ...
      } else if (lowerText.match(/^(✠\s*)?tin mừng chúa giê-su/i)) {
  ...
  if (!data.gospel.title || !data.gospel.lead || !data.gospel.content) {
    console.error("[VALIDATION FAILED] Phúc âm thiếu dữ liệu", Object.keys(data.gospel).filter(k => !data.gospel[k]));
  } else {
